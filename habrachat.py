...
define("redis_db", default=0, help="Redis db number")
...
class Subscriber(BaseHandler):
	__metaclass__ = Singleton

	def __init__(self):
		log.info("Subscribe init")
		self.send_client = self.redis()
		self.instance_id = _session_id()
		self.sub_client = self.redis()
	...
	def on_message(self, message):
		if message.kind == "message":
			...
			if chat_message["type"] == "get_all_users":
				...
				if not (
					have_remote_users(chat_message["user"]["id"], chat_message["user"]["hub"]) or
					have_local_users(chat_message["user"]["id"], chat_message["user"]["hub"])
				):
...
class ChatHandler(tornado.websocket.WebSocketHandler, BaseHandler):
	...
	def open(self):
		...
		habrachat_user = yield tornado.gen.Task(self.redis_client.get, habrachat_cookie)
		if habrachat_user:
			hub = self.get_argument("hub", options.hubs[0]["name"])
			real_hub_name = "main_room"
			for h in options.hubs:
				if h["name"] == hub:
					real_hub_name = hub
					break
			hub = real_hub_name
			...
			habrachat_user_settings = yield tornado.gen.Task(self.redis_client.get, "setting_"+habrachat_user["id"])
			...
			log.info("%s WebSocket opened by %s for hub:%s session_id:%s" % (
				self.subscriber.instance_id,
				habrachat_user["name"],
				hub,
				habrachat_user["session_id"]
			))
			...
			last_messages = yield tornado.gen.Task(self.redis_client.lrange, "hub_"+hub, 0, options.max_start_messages)
	...
	def on_message(self, message):
		...
		if message["type"] == "new_message":
			...
			yield tornado.gen.Task(self.redis_client.publish, "new_messages", new_message)
			...
			if my_user["ismoderator"]:
				...
				redis_messages = yield tornado.gen.Task(
					self.redis_client.lrange,
					"hub_"+my_user["hub"],
					0,
					options.max_save_messages
				)
				...
				if message_for_delete:
					...
					yield tornado.gen.Task(self.redis_client.lrem, key="hub_"+my_user["hub"],  num=-1, value=message_for_delete)
				else:
		...
		#		sockets.write_message(json_encode({"type":"active_chat_window", "user_id":my_user["id"]}))
			...
			if settings.get("revert_chat_order") is not None:
				my_user["settings"]["revert_chat_order"] = settings.get("revert_chat_order", False)
			if settings.get("send_message_enter") is not None:
				my_user["settings"]["send_message_enter"] = settings.get("send_message_enter", False)

			yield tornado.gen.Task(self.redis_client.set, "setting_"+my_user["id"],  json_encode(my_user["settings"]))
...
class MainHandler(tornado.web.RequestHandler, BaseHandler):
	...
	def get(self):
		...
		if habrachat_cookie:
			redis_client = self.redis()
			habrachat_user = yield tornado.gen.Task(redis_client.get, habrachat_cookie)
		if habrachat_user:
			self.render("chat.html", options=options)
		else:
			self.render("auth.html", options=options)
